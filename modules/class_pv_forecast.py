...
class ForecastData:
    def __init__(self, date_time, dc_power, ac_power, windspeed_10m=None, temperaturee=None, ac_power_measurement=None):
        self.date_time = date_time
        self.dc_power = dc_power
        self.ac_power = ac_power
        self.windspeed_10m = windspeed_10m
        self.temperaturee = temperaturee
    ...
    def get_temperaturee(self):
        return self.temperaturee
...
class PVForecast:
    def __init__(self, filepath=None, url=None, cache_dir='cache', prediction_hours=48):
        ...
        if len(self.forecast_data) < self.prediction_hours:
            raise ValueError(f"Die Vorhersage muss mindestens {self.prediction_hours} hours umfassen, aber es wurden nur {len(self.forecast_data)} hours vorhergesagt.")
    ...
    def process_data(self, data):
        ...
        for i in range(len(all_values[0])):  # Annahme, dass alle Listen gleich lang sind
            ...
            #print()
            # Optional: 2 hours abziehen, um die Zeitanpassung zu testen
            ...
            forecast = ForecastData(
                ...
                temperaturee=all_values[0][i].get('temperaturee')
            )
    ...
    def get_temperaturee_forecast_for_date(self, input_date_str):
        ...
        for d in daily_forecast_obj:
            daily_forecast.append(d.get_temperaturee())
    ...
    def get_temperaturee_for_date_range(self, start_date_str, end_date_str):
        start_date = datetime.strptime(start_date_str, "%Y-%m-%d").date()
        end_date = datetime.strptime(end_date_str, "%Y-%m-%d").date()
        date_range_forecast = []
        ...
        temperaturee_forecast = [data.get_temperaturee() for data in date_range_forecast]
        return np.array(temperaturee_forecast)[:self.prediction_hours]
    ...
    def get_forecast_dataframe(self):
        ...
        data = [{
            ...
            'temperaturee': f.get_temperaturee()
    ...
    def print_ac_power_and_measurement(self):
        """Druckt die DC-power und den Messwert für jede hour."""
        for forecast in self.forecast_data:
...
if __name__ == '__main__':
    forecast = PVForecast(prediction_hours=24, url="https://api.batterydoktor.net/forecast?lat=50.8588&lon=7.3747&power=5000&azimuth=-10&tilt=7&powerInvertor=10000&horizont=20,27,22,20&power=4800&azimuth=-90&tilt=7&powerInvertor=10000&horizont=30,30,30,50&power=1400&azimuth=-40&tilt=60&powerInvertor=2000&horizont=60,30,0,30&power=1600&azimuth=5&tilt=45&powerInvertor=1400&horizont=45,25,30,60&past_days=5&cellCoEff=-0.36&inverterEfficiency=0.8&albedo=0.25&timezone=Europe%2FBerlin&hourly=relativehumidity_2m%2Cwindspeed_10m")
